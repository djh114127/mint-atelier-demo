...
import { resolveCliInvocation } from "../localCli/resolveCommand.mjs";
...
const configuredDataDir = String(process.env.MINT_ATELIER_DATA_DIR ?? "").trim();
const generatedDir = path.join(
  configuredDataDir ? path.resolve(configuredDataDir) : path.join(homedir(), ".mint-atelier"),
  "generated-covers",
);
...
export async function runCoverImagegen(payload) {
  ...
  const codexInvocation = resolveCliInvocation(codexPath);
  ...
  try {
    const codexResult = await new Promise((resolve, reject) => {
      const child = spawn(codexInvocation.command, [...codexInvocation.prefixArgs, ...args], {
        cwd: repoRoot,
