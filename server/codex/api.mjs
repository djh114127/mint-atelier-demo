...
export function codexGenerateMiddleware() {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (requestUrl.pathname === "/api/cloud/config-status") {
        res.setHeader("Cache-Control", "no-store");
        sendJson(res, 200, {
          ok: true,
          channels: await getCloudSecretStatus(),
        });
        return;
      }

      if (requestUrl.pathname === "/api/cloud/config") {
        let saved;
        try {
          saved = await saveCloudApiKey(String(payload.channel ?? ""), payload.apiKey);
        } catch (error) {
          throw new CodexApiError("API_CONFIG_INVALID", error?.message || "API Key 配置无效。", 400);
        }
        res.setHeader("Cache-Control", "no-store");
