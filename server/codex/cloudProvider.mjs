...
import { resolveCloudApiKey } from "../config/cloudSecrets.mjs";
import {
  CodexApiError,
...
async function requireCloudConfig(payload, channel) {
  const modelName = String(payload.modelName ?? "").trim();
  const apiKey = await resolveCloudApiKey(channel, payload.apiKey);
...
export async function runCloudGeneration(payload) {
  const { modelName, apiKey, baseUrl } = await requireCloudConfig(payload, "text");
...
export async function runCloudCoverImage(payload) {
  const { modelName, apiKey, baseUrl } = await requireCloudConfig(payload, "image");
