...
export async function requestCloudConfigStatus() {
  return requestJson(
    "/api/cloud/config-status",
    {},
    {
      nonJson: "本机密钥状态服务返回了非 JSON 内容。",
      failed: "无法读取本机 API Key 保存状态。",
    },
    "API_CONFIG_STATUS_FAILED",
  );
}

export async function requestSaveCloudApiKey(channel, apiKey) {
  return requestJson(
    "/api/cloud/config",
    { channel, apiKey },
    {
      nonJson: "本机密钥保存服务返回了非 JSON 内容。",
      failed: "API Key 保存到本机失败。",
    },
    "API_CONFIG_SAVE_FAILED",
  );
}
