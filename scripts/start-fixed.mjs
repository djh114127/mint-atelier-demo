...
function resolveNpmInvocation() {
  ...
  if (!npmCliPath) {
    throw new Error("npm-cli.js was not found. Please reinstall Node.js with npm included.");
  }

  // Node 24 rejects direct shell:false execution of .cmd shims on Windows.
  // Invoking npm's JavaScript entry point keeps argument handling shell-free.
...
function installDependenciesIfNeeded() {
  ...
  const result = spawnSync(npmInvocation.command, [...npmInvocation.prefixArgs, "install"], {
    cwd: projectDir,
    stdio: "inherit",
    env: process.env,
    shell: false,
...
const server = run(npmInvocation.command, [...npmInvocation.prefixArgs, "run", npmScript], {
