...
云端演示构建必须确认 `dist/.openai/hosting.json`、`dist/server/index.js`、`dist/index.html` 和 `dist/assets/` 均存在。首次打开时不应请求 `/api/cloud/config-status`，点击“自动化生成”后应依次得到 6 条演示参考、3 条 RAG、10 个选题、5 篇文案、5 份 Prompt 和示例封面；搜索项来源必须显示“演示样例（非实时数据）”。
...
API Key 保存后只检查页面“已安全保存”状态或 `POST /api/cloud/config-status` 返回的 `hasApiKey`；该接口不得返回真实密钥。点击左侧“保存”后刷新页面，搜索结果、RAG、选题、文案、Prompt、封面和选中状态应完整恢复。
