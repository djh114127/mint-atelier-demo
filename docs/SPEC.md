...
产品同时提供免密钥演示模式。演示模式完全在浏览器中运行，不调用真实搜索、CLI 或云端 API；它提供明确标注的内置搜索样例、10 个选题、5 篇文案、5 份封面 Prompt、规则化候选选择和示例封面，用于面试与产品流程展示。演示内容不得被描述为实时平台数据或真实 AI 调用结果。
...
- 默认进入“演示模式”，无需密钥或本机依赖即可完成手动流程与自动化流程。
- 本地版本允许用户切换到“真实服务”，继续使用本机 CLI 或云端 API。
- 使用 `VITE_DEMO_ONLY=true` 构建的云端面试版本必须锁定演示模式，不显示可编辑的模型密钥配置，也不得请求本地 API 路由。
- 演示搜索结果必须显示“演示样例（非实时数据）”来源；状态区必须持续标识“免密钥演示引擎”。
- 模式切换仍由用户主动操作；切换模式不得静默触发搜索、生成或入库。
...
用户点击某个封面 Prompt 后，系统按图片模型配置生成对应 PNG 封面图。本地路线通过 Codex CLI worker 调用 native `image_gen.imagegen`；云端路线通过 OpenAI-compatible `POST /images/generations` 生成图片。两条路线都需要把生成结果展示在页面中，图片文件持久保存到当前用户的 `.mint-atelier/generated-covers/`，并由本地 Vite middleware 托管在 `/generated/covers/*.png`。生成失败时，需要保留原始封面 Prompt，方便用户重新生成。
...
非敏感模型配置需要自动缓存。API Key 不得在前端长期存储或回显，必须通过本机服务端保存到当前用户的 `.mint-atelier/secrets.json`；前端只读取各通道的 `hasApiKey` 状态。旧版 `localStorage` 中的 API Key 需要一次性迁移：先确认服务端保存成功，再从浏览器存储移除，迁移失败时保留原值并给出明确提示。

### 3.11 草稿持久化

用户点击左侧“保存”后，需要把当前阶段、搜索结果、搜索勾选项、RAG 参考、选题与选中项、文案与选中项、封面 Prompt 与选中项、封面结果和保存时间写入当前 origin 的 `localStorage`。页面刷新后恢复完整工作状态并允许继续编辑；API Key 不得进入草稿快照。
